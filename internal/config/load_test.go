...
func TestLoad_WithConfigFiles(t *testing.T) {
	...
	for _, filename := range formats {
		t.Run(filename, func(t *testing.T) {
			...
			asserts.RelativeEq(t, 300/25.4, cfg.SvgCanvasResolution)
			asserts.RelativeEq(t, 60.0, cfg.VideoFps)
...
func TestLoad_UsesOverrides(t *testing.T) {
	...
	v.Set("max_archive_file_bytes", 2*units.GiB)
	v.Set("max_pdf_file_bytes", 32*units.MiB)
	v.Set("max_text_file_bytes", 1*units.MiB)
